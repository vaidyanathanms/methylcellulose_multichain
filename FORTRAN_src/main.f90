...
SUBROUTINE READ_ANA_IP_FILE()
  ...
  DO
     ...
     ELSEIF(dumchar == 'compute_eigvals') THEN
        ...
        READ(anaread,*,iostat=ierr) eigcalc, indeig, globeig
        IF(eigcalc .OR. indeig) eigcalc  = 1
     ...
     ELSEIF(dumchar == 'compute_diff') THEN

        !diff_nframes is different from nframes
        !required when diff_nframes > nframes
        READ(anaread,*,iostat=ierr) diff_nframes
        diffcalc = 1
...
SUBROUTINE DEFAULTVALUES()
  ...
  nframes = 0; skipfr = 0; nwater = 0; nchains = 0
  atperchain = 0; nproc = 0

  ! Initialize Flags
  rgall = 0; rgcalc = 0;rdfcalc = 0; oxycalc = 0; rescalc = 0
  eigcalc = 0; rgsys = 0; denscalc = 0; indeig = 0;globeig=0
  diffcalc = 0
  
  ! Initialize distributions and frequencies
  rdffreq = 0; rgfreq = 0; oxyfreq = 0;  rg_s_freq=0
  densfreq = 0; dens_axis = 0; ndentypes = 0
  ...
  ! Initialize Extra Dynamical Quantities
  diff_nframes = 0
...
!$OMP END PARALLEL
     ...
     IF (diffcalc) THEN
        ...
        IF (i > diff_nframes) THEN
           
           PRINT *, "nframes/diff_nframes", nframes,diff_nframes
           PRINT *, "nframes should be atleast diff_nframes"
           STOP
        ...
        DO j = 1,atchk
           ...
           rx_time(j,i) = rxyz_lmp(j,1)
           ry_time(j,i) = rxyz_lmp(j,2)
           rz_time(j,i) = rxyz_lmp(j,3)
  ...
  IF (diffcalc) CALL COMPUTE_DIFFUSIVITY()
...
SUBROUTINE OPEN_STRUCT_OUTPUT_FILES()
  ...
  IF(globeig) THEN
     dum_fname = "globeig_"//trim(adjustl(traj_fname))
     OPEN(unit = eigallwrite,file =trim(dum_fname),action="write"&
          &,status="replace")
  END IF
...
SUBROUTINE STRUCT_INIT()
  ...
  IF(oxycalc) THEN
     ...
     PRINT *, "Number of oxygen types: ", nwater

     ALLOCATE(oxyarray(nwater),stat = AllocateStatus)
     ...
     IF(j .NE. nwater) THEN
        ...
        PRINT *, "Oxyarray not counted correctly", j, nwater
        STOP
  ...
  IF(rescalc) THEN
     ...
     ALLOCATE(autocf(nwater,nframes),stat = AllocateStatus)
     IF(AllocateStatus/=0) STOP "did not allocate autocf"
     ALLOCATE(oxyresarray(nwater),stat = AllocateStatus)
     ...
     IF(j .NE. nwater) THEN
        ...
        PRINT *, "Oxyresarray not counted correctly", j, nwater
        STOP
...
SUBROUTINE STRUCT_MAIN(tval)
  ...
  IF(rescalc .AND. tval == nframes) CALL COMPUTE_SPECTRA()
  IF(indeig .OR. eigcalc) CALL COMPUTE_EIGVALS(tval)
  IF(globeig) CALL COMPUTE_GLOBEIGVALS(tval)
...
SUBROUTINE COMPUTE_RADGYR(iframe)
  ! An important distinction between the radius of gyration obtained
  ! from the calculation here and that from the sum of the eigenvalues
  ! is that the Rg computed here is weighted by mass and the sum of
  ! the eigenvalues is not weighted by mass.
  USE PARAMS_SOLVATEDMC
  IMPLICIT NONE
  ...
  IF(iframe == 1) THEN
     ...
     DO i = 1,nchains-1
        ...
        r2com = (rxcm(i)-rxcm(i+1))**2 +(rycm(i)-rycm(i+1))**2 + &
             & (rzcm(i)-rzcm(i+1))**2

        WRITE(98,'(2(I0,1X),F14.9)') i,i+1,sqrt(r2com)
  ...
  DO i = 1,nchains*atperchain
     ...
     rgxx(molid) = rgxx(molid) + ((rxyz_lmp(i,1)&
          &-rxcm(molid))**2)*masses(atype,1)
     rgyy(molid) = rgyy(molid) + ((rxyz_lmp(i,2)&
          &-rycm(molid))**2)*masses(atype,1)
     rgzz(molid) = rgzz(molid) + ((rxyz_lmp(i,3)&
          &-rzcm(molid))**2)*masses(atype,1)

     rgsq(molid) = rgsq(molid) + ((rxyz_lmp(i,1)&
          &-rxcm(molid))**2 + (rxyz_lmp(i,2)-rycm(molid))**2 +&
          & (rxyz_lmp(i,3)-rzcm(molid))**2)*masses(atype,1)
...
SUBROUTINE COMPUTE_OXYCLUS(iframe)
  ...
  INTEGER, INTENT(IN) :: iframe
  INTEGER :: AllocateStatus
  INTEGER :: i,j,a1id,molid,atcnt,cnt,a2id,chcnt
  REAL :: rxval, ryval, rzval, rval
  INTEGER, ALLOCATABLE, DIMENSION(:) :: inst_avg_num_chains

  ALLOCATE(inst_avg_num_chains(nwater),stat = AllocateStatus)
...
!$OMP PARALLEL DO PRIVATE(i,j,rxval,ryval,rzval,rval,a1id,molid, &
!$OMP& chcnt,atcnt,a2id)
  ...
  DO i = 1,nwater
     ...
     DO chcnt = 1,nchains
        ...
        DO WHILE(atcnt .LE. atperchain)
           
           j = (chcnt-1)*atperchain + atcnt
...
SUBROUTINE COMPUTE_RESTIME(iframe)
  ...
  INTEGER, INTENT(IN) :: iframe
  INTEGER :: i,j,tval,a1id,a2id,ierr,ifin,chcnt
  REAL :: rxval,ryval,rzval,rval

!$OMP PARALLEL DO PRIVATE(i,j,chcnt,rxval,ryval,rzval,rval,a1id,a2id)
  DO i = 1,nwater !populate autocorrelation fn array

     a1id = oxyresarray(i)
     
     DO WHILE(chcnt .LE. nchains)
        
        DO WHILE(j .LE. atperchain)
           
           a2id = (chcnt-1)*atperchain + j
...
!$OMP PARALLEL DO PRIVATE(tinc,ifin,tim,i,j)
  ...
  DO tinc = 0, nframes-1 !compute spectral product
     ...
     tplot_cf(tinc) = tplot_cf(tinc)/REAL(ifin*nwater)
...
SUBROUTINE COMPUTE_EIGVALS(iframe)
  ! An important distinction between the radius of gyration obtained
  ! from the calculation here and that from the COMPUTE_RADGYR module
  ! is that the Rg computed there is weighted by mass and the sum of
  ! the eigenvalues is not weighted by mass.
  ...
  REAL :: SMN_ALL(N_LA,3)
  REAL :: rxcm, rycm, rzcm
  REAL,DIMENSION(1:atperchain,3) :: rshift_lmp 

  IF(indeig)  WRITE(eigwrite,'(2(I0,1X))') iframe, nchains
  ...
  DO i = 1,nchains
     ...
     rxcm = 0.0; rycm = 0.0; rzcm = 0.0
     CALL COMPUTE_CENTROID(i,rxcm,rycm,rzcm)
          
     DO j = 1, atperchain
        
        a1id = (i-1)*atperchain + j
     ...
     IF(indeig /= 0) THEN
        ...
        WRITE(eigwrite,'(I0,1X,3(F14.6,1X))') i, W(1), W(2), W(3)
...
!--------------------------------------------------------------------
SUBROUTINE COMPUTE_GLOBEIGVALS(iframe)
  ! Note 1: An important distinction between the radius of gyration
  ! obtained from the calculation here and that from the
  ! COMPUTE_RADGYR module is that the Rg computed there is weighted
  ! by mass and the sum of the eigenvalues is not weighted by mass.

  ! Note 2: This is different from averaging over different chains.
  ! Here the entire system is considered as one entity and then the
  ! COM/Rg is computed. This computes the global distribution of
  ! points.
  ...
  REAL :: SMN_ALL(N_LA,3)
  REAL :: rxcm, rycm, rzcm
  REAL,DIMENSION(1:nchains*atperchain,3) :: rshift_lmp 
  ...
  rshift_lmp = 0.0; SMN = 0.0
  rxcm = 0.0; rycm = 0.0; rzcm = 0.0
  CALL COMPUTE_CENTROID(-1,rxcm,rycm,rzcm)
  ...
  SMN = SMN/REAL(atperchain*nchains)
  ...
  ! Create Eigen Value Matrix for each chain
  CALL DSYEV('N','U',N_LA,SMN,N_LA,W,WORK,LWORK,INFO)
  WRITE(eigallwrite,'(I0,1X,3(F14.6,1X))') nchains, W(1), W(2), W(3)
  IF(INFO .NE. 0) STOP 'Physically irrelevant Eigenvalues'
     
END SUBROUTINE COMPUTE_GLOBEIGVALS
...
SUBROUTINE COMPUTE_COM(chval,rxcm,rycm,rzcm)

  USE PARAMS_SOLVATEDMC
  IMPLICIT NONE

  INTEGER :: i,j,atype,a1id
  INTEGER,INTENT(IN) :: chval
  REAL,INTENT(OUT) :: rxcm,rycm,rzcm
  REAL :: mtype,rx,ry,rz,totmass
  ...
  IF(chval .NE. -1) THEN
     ...
     DO i = 1,atperchain
        ...
        a1id = (chval-1)*atperchain + i
        atype = aidvals(a1id,3)
        mtype = masses(atype,1)
        rxcm = rxcm + rxyz_lmp(a1id,1)*mtype
        rycm = rycm + rxyz_lmp(a1id,2)*mtype
        rzcm = rzcm + rxyz_lmp(a1id,3)*mtype
        totmass = totmass + mtype
  ...
  ELSE !if chval == -1, then global COM is computed

     DO j = 1,nchains

        DO i = 1,atperchain

           a1id = (j-1)*atperchain + i
           atype = aidvals(a1id,3)
           mtype = masses(atype,1)
           rxcm = rxcm + rxyz_lmp(a1id,1)*mtype
           rycm = rycm + rxyz_lmp(a1id,2)*mtype
...
SUBROUTINE COMPUTE_CENTROID(chval,rxcm,rycm,rzcm)
  ...
  IF(chval .NE. -1) THEN
     ...
     rxcm = rxcm/REAL(atperchain)
     rycm = rycm/REAL(atperchain)
     rzcm = rzcm/REAL(atperchain)
  ...
  ELSE
     ...
     rxcm = rxcm/REAL(atperchain*nchains)
     rycm = rycm/REAL(atperchain*nchains)
     rzcm = rzcm/REAL(atperchain*nchains)
...
SUBROUTINE COMPUTE_DIFFUSIVITY()
  ...
  REAL    :: rxcm, rycm, rzcm
  REAL, DIMENSION(0:diff_nframes-1) :: gxarr,gyarr,gzarr

  OPEN(unit = 91,file = "diff.txt", status="replace",action&
       &="write",iostat = ierr)

  IF(ierr /= 0) STOP "Diffusion file not found"

  PRINT *, "Computing diffusivity... "

! Ion Diffusion Analysis

  DO i = 0,diff_nframes-1
  ...
  DO tinc = 0, diff_nframes-1
     ...
     DO i = 1,ifin
        ...
        DO j = 1,ntotatoms
           ...
           rxcm = rx_time(j,tim) - rx_time(j,i)
           rycm = ry_time(j,tim) - ry_time(j,i)
           rzcm = rz_time(j,tim) - rz_time(j,i)
...
SUBROUTINE OUTPUT_OXYCLUS()
  ...
  DO i = 0, nchains
     ...
     WRITE(dumwrite,*) i, histoxyarray(i)/REAL(nwater*oxynorm)
...
! Allocate Box details
  ...
  IF(rdfcalc) THEN
     ALLOCATE(rdfarray(rmaxbin,npairs),stat = AllocateStatus)
  ...
  IF(diffcalc == 0) THEN
     ALLOCATE(rx_time(1,1),stat = AllocateStatus)
     IF(AllocateStatus/=0) STOP "did not allocate rx_time"
     DEALLOCATE(rx_time)
     ALLOCATE(ry_time(1,1),stat = AllocateStatus)
     IF(AllocateStatus/=0) STOP "did not allocate ry_time"
     DEALLOCATE(ry_time)
     ALLOCATE(rz_time(1,1),stat = AllocateStatus)
     IF(AllocateStatus/=0) STOP "did not allocate rz_time"
     DEALLOCATE(rz_time)
  ELSE
     ALLOCATE(rx_time(ntotatoms,diff_nframes),stat = AllocateStatus)
     IF(AllocateStatus/=0) STOP "did not allocate rx_time"
     ALLOCATE(ry_time(ntotatoms,diff_nframes),stat = AllocateStatus)
     IF(AllocateStatus/=0) STOP "did not allocate ry_time"
     ALLOCATE(rz_time(ntotatoms,diff_nframes),stat = AllocateStatus)
     IF(AllocateStatus/=0) STOP "did not allocate rz_time"
  END IF
